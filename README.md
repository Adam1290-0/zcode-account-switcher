...
|---|---|---|---|
| **v1.0.2（最新）** | **3.10.1 / 3.10.2 / 3.11.2 / 3.12.2 / 3.12.3** | ✅ 当前维护版本 | **修复切换不完整**：套餐/额度状态（config builtin token、providerFamily、套餐缓存）随账号一并切换；推广链接带邀请后缀 |
| v1.0.1 | 3.10.1 / 3.10.2 | ✅ | 模型设置页「切换账号」按钮 + 广告横幅；`/api/diag` 诊断 |
...
- 🆙 验证适配 ZCode 3.10.2 / 3.11.2 / 3.12.2 / 3.12.3（主进程/渲染层/全部文本锚点未变）
- 🔧 patch.bat 备份改为自动刷新：ZCode 更新后（asar 为干净版）重打补丁时，备份同步为新版本，卸载补丁不会再把应用降回旧版
